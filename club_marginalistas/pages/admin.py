...
def create_form() -> rx.Component:
    return panel(
        rx.vstack(
            ...
            form_field("Contenido (Markdown)", rx.text_area(value=AdminState.content, on_change=AdminState.set_content, placeholder="Escribe el contenido en Markdown...", **textarea_style)),
...
def posts_list() -> rx.Component:
    return panel(
        rx.vstack(
            ...
            rx.foreach(
                AdminState.posts,
                lambda post: post_list_item(post, lambda: AdminState.delete_existing_post(post.slug)),
            ),
